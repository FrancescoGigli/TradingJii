...
def render_backtest_tab():
    ...
    # ═══════════════════════════════════════════════════════════════════
    # DUAL SIGNAL DISPLAY: Signal Calculator + XGB Model
    # ═══════════════════════════════════════════════════════════════════
    st.markdown("---")
    ...
    with col_xgb:
        ...
        if ml_service.is_available and ml_prediction and ml_prediction.is_valid:
            ...
            # Determine main signal (use LONG score as primary, negated SHORT as alternative)
            # Positive normalized score = LONG opportunity, negative = SHORT opportunity
            combined_xgb = xgb_score_long - xgb_score_short  # LONG bias - SHORT bias
            ...
            # Main score display (use LONG normalized for display)
            st.markdown(f"""
            <div style="background: linear-gradient(135deg, #1a1a2e 0%, #16213e 100%); 
            ...
            # XGB gauge (same style as technical)
    ...
    if ml_service.is_available and ml_prediction and ml_prediction.is_valid:
        ...
        with col3:
            ...
            <div style="background: #1e1e2e; padding: 12px; border-radius: 8px; text-align: center; border-left: 3px solid {agreement_color};">
                <p style="color: #888; margin: 0 0 5px 0; font-size: 0.8rem;">{agreement_icon} Status</p>

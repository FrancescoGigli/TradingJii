...
CREATE TABLE IF NOT EXISTS position_stops (
    ...
    side                    TEXT NOT NULL,
    current_stop_price      NUMERIC(30, 10) NOT NULL,
    stop_type               TEXT NOT NULL,
    highest_profit_pct      NUMERIC(10, 6),
    leverage                INTEGER DEFAULT 10,
    is_active               BOOLEAN DEFAULT TRUE
);

CREATE INDEX IF NOT EXISTS idx_position_stops_symbol
    ON position_stops(symbol, is_active);

CREATE TABLE IF NOT EXISTS stop_loss_closures (
    id                  BIGSERIAL PRIMARY KEY,
    created_at          TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    symbol              TEXT NOT NULL,
    side                TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_stop_loss_closures_created_at
    ON stop_loss_closures(created_at);
"""
...
def log_bot_operation(
    ...
    *,
    ...
    Restituisce l'ID dell'operazione creata (o dell'ultima operazione se multiple).
    """

    # Normalized list of operations
    ops_list = []
    if "decisions" in operation_payload and isinstance(operation_payload["decisions"], list):
        ops_list = operation_payload["decisions"]
    elif "operation" in operation_payload:
        ops_list = [operation_payload]
    else:
        # Se è vuoto o formato non riconosciuto, potremmo loggare un no-op o raise
        # Per compatibilità, se manca operation raise ValueError solo se non ci sono decisions
        raise ValueError("operation_payload deve contenere 'operation' o 'decisions'")
    ...
    with get_connection() as conn:
        with conn.cursor() as cur:
            ...
            # 3) Operazione del bot collegata al contesto (multiple ops possibili)
            for op_item in ops_list:
                op_op = op_item.get("operation")
                op_sym = op_item.get("symbol")
                op_dir = op_item.get("direction")
                op_tgt = op_item.get("target_portion_of_balance")
                op_lev = op_item.get("leverage")
...
def log_stop_loss_closures(closures: List[Dict[str, Any]]) -> None:
    """Logga le chiusure di posizioni per stop loss.
    
    Parametri:
    - closures: lista di dict con dettagli delle chiusure, es:
        [
            {
                "symbol": "BTC",
                "side": "long",
    ...
    with get_connection() as conn:
        with conn.cursor() as cur:
            for closure in closures:
                cur.execute(
                    """
                    INSERT INTO stop_loss_closures (
                        symbol,
                        side,
                        entry_price,
                        close_price,
                    ...
                    (
                        closure.get("symbol"),
                        closure.get("side"),
                        closure.get("entry_price"),
                        closure.get("close_price"),
                        closure.get("size"),
                        closure.get("profit_pct"),
                        closure.get("stop_type"),
                        closure.get("reason"),
                        Json(closure),
                    ),
                )

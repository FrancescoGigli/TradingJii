...
class HyperLiquidTrader:
    ...
    def get_top_liquid_coins(self, limit: int = 20) -> list[str]:
        """
        Ottiene la lista dei ticker ordinati per volume nelle ultime 24h (dayNtlVlm).
        Restituisce una lista di stringhe, es: ['BTC', 'ETH', 'SOL', ...]
        """
        try:
            ...
            # Prendi i top N
            top_list = [s[0] for s in stats[:limit]]
            print(f"✅ Top {limit} coins by volume fetchati con successo: {top_list}")
            return top_list
            
        except Exception as e:
            print(f"❌ Errore fetchando top coins: {e}")
            # Fallback sicuro
    ...
    def _validate_order_input(self, order_json: Dict[str, Any]):
        ...
        # Allow empty direction if operation is 'hold' or 'close'
        # (market_close automatically closes the open position)
        if order_json["operation"] in ("hold", "close"):
            pass
        elif order_json["direction"] not in ("long", "short"):
    ...
    def get_current_leverage(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Se non c'è posizione aperta, non possiamo sapere lo stato specifico
            # ma se abbiamo appena chiamato set_leverage_for_symbol(is_cross=False),
            # l'ordine successivo userà Isolated.
            cross_leverage = user_state.get('crossLeverage', 20)
            return {
                'value': cross_leverage,
                'type': 'cross (default)',
                'coin': symbol,
                'note': 'No open position found. Next order will respect set_leverage settings (Isolated 10x).'
            }
    ...
    def execute_signal(self, order_json: Dict[str, Any]) -> Dict[str, Any]:
        ...
        leverage_result = self.set_leverage_for_symbol(
            symbol=symbol,
            leverage=leverage,
            is_cross=False  # Changed to False for Isolated Margin as requested
        )
    ...
    #                           STORICO PnL
    # ----------------------------------------------------------------------
    def close_position_with_reason(self, symbol: str, reason: str) -> Dict[str, Any]:
        """
        Chiude una posizione e aggiunge il motivo della chiusura ai metadati
        
        Args:
            symbol: simbolo della posizione da chiudere
            reason: motivo della chiusura (es. "fixed_stop_loss", "trailing_stop_loss")
        
        Returns:
            Dict con risultato dell'exchange e reason
        """
        print(f"[HyperLiquidTrader] Chiusura posizione {symbol} - Motivo: {reason}")
    ...
    def get_recent_performance(self) -> str:
        """
        Recupera gli ultimi trade chiusi e il relativo PnL.
        Restituisce una stringa di riepilogo per l'AI.
        """
        try:
            ...
            # Filtra solo quelli con closedPnl (cioè chiusure)
            closed_trades = [f for f in fills if f.get("closedPnl")]
            
            if not closed_trades:
                return "No recent closed positions to analyze performance."
            
            # Prendi ultimi 10
            recent = closed_trades[:10]
            
            summary = "RECENT CLOSED TRADES PERFORMANCE:\n"
            total_pnl = 0.0
            ...
            for t in recent:
                ...
                total_pnl += pnl
                # side 'B' means we Bought to close (so we were Short). 
                # dir usually says "Close Short" etc.
                direction = t.get('dir', 'Close')
                summary += f"- {coin} ({direction}): PnL ${pnl:+.4f}\n"
            
            summary += f"\nTotal PnL (last 10): ${total_pnl:+.4f}"
            return summary
            
        except Exception as e:
            print(f"Errore recuperando storico PnL: {e}")
            return "Error retrieving recent performance."
